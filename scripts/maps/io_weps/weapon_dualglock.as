...
class weapon_dualglock : ScriptBasePlayerWeaponEntity
{
	...
	void PrimaryAttack()
	{
		...
		else
		{
			...
			iAnimation += g_PlayerFuncs.SharedRandomLong( self.m_pPlayer.random_seed, 0, 4 );
		}
		
		self.SendWeaponAnim( iAnimation, 0, 0 );
		
		g_SoundSystem.EmitSoundDyn( self.m_pPlayer.edict(), CHAN_WEAPON, "rng/kuilu/weapons/glock_fire.ogg", 0.9, ATTN_NORM, 0, PITCH_NORM );
		...
		if( iAnimation == ELITES_SHOOTRIGHT1 )
			GetDefaultShellInfo( self.m_pPlayer, vecShellVelocity, vecShellOrigin, 21, -9, -7 );
	...
	void Reload()
	{
		...
		self.m_pPlayer.m_szAnimExtension = "uzis";
	}
